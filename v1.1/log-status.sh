...
if test $o10 -eq "0"  
then 
echo -e "[${Bold_Green:-}OK${Reset:-}] Start-status Fulfillment Server for $SID on port 9300"
else 
echo -e "[${Bold_Red:-}FAILED${Reset:-}] Start-status Fulfillment Server for $SID on port 9300"
fi
#---#
if test $o11 -eq "0"  
then 
...
if test $i1 -eq "0"  
then 
echo -e "[${Bold_Green:-}OK${Reset:-}] Stop-status Fulfillment Server for $SID on port 9300"
else 
echo -e "[${Bold_Red:-}FAILED${Reset:-}] Stop-status Fulfillment Server for $SID on port 9300"
fi
#---#
if test $i2 -eq "0"  
then 
